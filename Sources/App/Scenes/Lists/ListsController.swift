...
final class ListsController: ProtectedController, SortingController, RouteCollection {
        ...
        -> Future<[ListContext]>
    {
        ...
        return try request.make(ListRepository.self)
            ...
            .flatMap { (lists) throws -> Future<[ListContext]> in
                return try lists.map { (list) throws -> Future<ListContext> in
                    var context = ListContext(for: list)
                    return try request.make(ItemRepository.self).count(on: list).map { count in
    ...
    private static func renderView(on request: Request) throws -> Future<View> {
        ...
        return try ListsController.buildContexts(for: user, on: request).flatMap {
